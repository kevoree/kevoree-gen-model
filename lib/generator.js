...
function genHashcode(pkg, callback) {
  var data = pkg.kevoree.namespace + '/' + pkg.name + '/' + pkg.version;
  pkg.dependencies = pkg.dependencies || {};
  async.parallel(
    Object.keys(pkg.dependencies)
      .map(function (dep) {
        return function (done) {
          if (semver.validRange(pkg.dependencies[dep])) {
            packageJson(dep, pkg.dependencies[dep])
              .then(function (json) {
                done(null, json);
              })
              .catch(function (err) {
                err.message += ' ('+dep + '@' + pkg.dependencies[dep]+')';
                done(err);
              });
          } else {
            packageJson(dep)
              .then(function (json) {
                packageJson(dep, json['dist-tags'][pkg.dependencies[dep]])
                  .then(function (json) {
                    done(null, json);
                  })
                  .catch(function (err) {
                    err.message += ' ('+dep + '@' + json[pkg.dependencies[dep]]+')';
                    done(err);
                  });
              })
              .catch(function (err) {
...
var generator = function generator(dirPath, quiet, callback) {
  ...
  if (pkgJson.kevoree && typeof (pkgJson.kevoree.namespace) === 'string') {
    if (PKG_PATTERN.test(pkgJson.kevoree.namespace)) {
      ...
      genHashcode(pkgJson, function (err, hashcode) {
        if (err) {
          callback(err);
        } else {
          deployUnit.hashcode = hashcode;
          ...
          if (Class.prototype.tdef_version) {
            ...
            callback(null, model, pkgJson.kevoree.namespace, leafPkg.typeDefinitions.get(0), deployUnit);
          } else {
