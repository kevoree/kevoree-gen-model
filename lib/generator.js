...
var generator = function generator(dirPath, quiet, callback) {
  ...
  if (pkgJson.kevoree && typeof (pkgJson.kevoree.namespace) === 'string') {
    if (NAMESPACE_PATTERN.test(pkgJson.kevoree.namespace)) {
      ...
      pkgHash(dirPath, function (err, hash) {
        if (err) {
          callback(err);
        } else {
          deployUnit.hashcode = crypto.createHash('md5')
            .update(hash + pkgJson.kevoree.namespace + '/' + pkgJson.name + '/' + pkgJson.version + JSON.stringify(pkgJson.dependencies))
            .digest('hex');
